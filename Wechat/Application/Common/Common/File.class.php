...
class File {
    ...
    */
     ...
     *$file2 旧的内容
     */
    public function processMoveFilesFromTmpsToStorage($file1,$file2,$dirname='System'){
    	if(!$file1){
           	//新内容关联文件为空则只需要删除旧内容关联文件
           	$this->processDeleFiles($file2);
        }elseif(!$file2){
        	//旧内容关联文件为空则移动新内容关联文件
        	$file1=$this->moveFilesFromTmpsToStorage($file1,$dirname);
        }elseif($file1!=$file2){
        	$file1=$this->moveFilesFromTmpsToStorage($file1,$dirname);
        	$this->processDeleFiles($file2);
        }
     ...
     *$file 多个文件约定用逗号分隔
     */
    private function moveFilesFromTmpsToStorage($file_param,$dirname='System'){
    	if(!$file_param){
    		return ;
    	}
    	...
    	foreach($files as $file){
	        ...
	        if(file_exists($file)){
	            //replace text
	            $toFile=str_replace('Tmps', $dirname, $file);
    	...
    	return preg_replace('/\/+Uploads\/+Tmps/i', '/Uploads/'.$dirname, $file_param);
    }
    ...
    public function saveBase64Files($base64_url,$save_tmps=0){
        if(!$base64_url){
            return ;
        }
        if(!preg_match('/^data:\s*image\/\w+;base64/', $base64_url, $matches)){
            return ;
        }
        //文件格式
        $format=preg_replace('/data:\s*image\//', '', $matches[0]);
        $format=str_replace(';base64', '', $format);
        $file_name=generateUniqid().'.'.$format;

        if(!$save_tmps){
            $attachDirs=C('CACHE_DIR').'/Uploads/Common';
            $fileAccessLinkUrl='/Uploads/Common';
        }else{
            $attachDirs=C('CACHE_DIR').'/Uploads/Tmps';
            $fileAccessLinkUrl='/Uploads/Tmps';
        }
        
        $attachDirs=$attachDirs.'/'.date('Y').'/'.date('m').'/'.date('d');
        $fileAccessLinkUrl=$fileAccessLinkUrl.'/'.date('Y').'/'.date('m').'/'.date('d');

        if(!is_dir($attachDirs)){
            @mkdir($attachDirs,0777,true);
        }
        //去除头部
        $base64_image = substr(strstr($base64_url,','),1);
        //解码
        $data= base64_decode($base64_image);
        //存储
        $file_path=$attachDirs.'/'.$file_name;
        $link_path=$fileAccessLinkUrl.'/'.$file_name;
